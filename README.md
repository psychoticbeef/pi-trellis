...
Innerhalb eines **Story-Fensters** meldet die Extension **Token-Usage** laufend am Turn-Ende über `trellis usage add`. **Assistant-Usage** und Usage der **pi-subagents-Records** aus `subagents:completed` und `subagents:failed` werden nach `input`, `output`, `cacheRead` und `cacheWrite` getrennt gemeldet. Fehlen vollständige, mit `totalTokens` abstimmbare Kategorien, nutzt das einzelne Usage-Objekt den **Legacy-Fallback** über `--main` oder `--subagents`. Kategorisierte und Legacy-Anteile laufen in getrennten CLI-Aufrufen. Erfolgreich gemeldete **Usage-Deltas** werden nicht erneut gesendet. CLI-Fehler erzeugen nur eine UI-Warnung; nur das ungemeldete Usage-Delta bleibt für nächsten Turn-End-Versuch erhalten.
